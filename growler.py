...
def notifier(message="", title="", sound = False):
...
try:
    try:
        ...
        def notify(content="", title="Security Notification", icon="/Applications/Utilities/Network Utility.app"):
            if content:
        ...
        def alert(content="", title="SECURITY ALERT", icon="/Applications/Utilities/Network Utility.app"):
            if content:
    ...
    try:
        def notify(content="", title="Security Notification", icon="http://i.imgur.com/auYfC7O.png"):
            notifier(content, title)
        def alert(content="", title="SECURITY ALERT", icon="http://i.imgur.com/auYfC7O.png"):
            notifier(content, title)

    except Exception as e:
        print "[X] Error initializing: %s" % e
        print "[!] OS X notifications not supported. Requires OS 10.8 or higher"
        def notify(content="", title="Security Notification"):
            if content:
                print "[>] secnotify[%s]: %s" % (title, content)
        def alert(content="", title="SECURITY ALERT"):
            if content:
                print "[>] secalert[%s]: %s" % (title, content)
            ...
            if not (secure_line or apache_line or ftp_line):
                ...
                if not first_run:
                    if secure_line:
                        ...
                        notify(content=secure_line, title="SSH & Sudo")
                    if apache_line:
                        apache_line = parse(line=apache_line, log_type="apache")
                        notify(content=apache_line, title="Apache2 Webserver")
                    if ftp_line:
                        ftp_line = parse(line=ftp_line, log_type="ftp")
                        notify(ftp_line, title="FTP")
